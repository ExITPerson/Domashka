from src.generators import filter_by_currency, transaction_descriptions, card_number_generator


def test_filter_by_currency(transactions):
    usd_transactions = filter_by_currency(transactions, "USD")
    assert next(usd_transactions) == {
            "id": 939719570,
            "state": "EXECUTED",
            "date": "2018-06-30T02:08:58.425572",
            "operationAmount": {
                "amount": "9824.07",
                "currency": {
                    "name": "USD",
                    "code": "USD"
                }
            },
            "description": "Перевод организации",
            "from": "Счет 75106830613657916952",
            "to": "Счет 11776614605963066702"
        }
    assert next(usd_transactions) == {
            "id": 142264268,
            "state": "EXECUTED",
            "date": "2019-04-04T23:20:05.206878",
            "operationAmount": {
                "amount": "79114.93",
                "currency": {
                    "name": "USD",
                    "code": "USD"
                }
            },
            "description": "Перевод со счета на счет",
            "from": "Счет 19708645243227258542",
            "to": "Счет 75651667383060284188"
        }
    assert next(usd_transactions) == {
            "id": 895315941,
            "state": "EXECUTED",
            "date": "2018-08-19T04:27:37.904916",
            "operationAmount": {
                "amount": "56883.54",
                "currency": {
                    "name": "USD",
                    "code": "USD"
                }
            },
            "description": "Перевод с карты на карту",
            "from": "Visa Classic 6831982476737658",
            "to": "Visa Platinum 8990922113665229"
        }

    rub_transactions = filter_by_currency(transactions, "RUB")
    assert next(rub_transactions) == {
            "id": 873106923,
            "state": "EXECUTED",
            "date": "2019-03-23T01:09:46.296404",
            "operationAmount": {
                "amount": "43318.34",
                "currency": {
                    "name": "руб.",
                    "code": "RUB"
                }
            },
            "description": "Перевод со счета на счет",
            "from": "Счет 44812258784861134719",
            "to": "Счет 74489636417521191160"
        }
    assert next(rub_transactions) =={
            "id": 594226727,
            "state": "CANCELED",
            "date": "2018-09-12T21:27:25.241689",
            "operationAmount": {
                "amount": "67314.70",
                "currency": {
                    "name": "руб.",
                    "code": "RUB"
                }
            },
            "description": "Перевод организации",
            "from": "Visa Platinum 1246377376343588",
            "to": "Счет 14211924144426031657"
        }


def test_filter_by_currency_empty_list():
    assert next(filter_by_currency([], 'USD')) == 'Транзакций не обнаружено'
    assert next(filter_by_currency([{}], 'USD')) == 'Транзакций не обнаружено'


def test_filter_by_not_currency():
    assert next(filter_by_currency([{"id": 549386222,
            "state": "EXECUTED",
            "date": "2019-04-04T23:20:05.206878",
            "operationAmount": {
                "amount": "548326.79",
                "currency": {"name": "RUB","code": "RUB"}},
            "description": "Оплата в магазине",
            "from": "Счет 10798645567227258432",
            "to": "Счет 75651667383060284188"
        }], 'USD')) == 'Транзакций не обнаружено'


def test_transaction_descriptions(transactions):
    name_transactions = transaction_descriptions(transactions)
    assert next(name_transactions) == "Перевод организации"
    assert next(name_transactions) == "Перевод со счета на счет"
    assert next(name_transactions) == "Перевод со счета на счет"
    assert next(name_transactions) == "Перевод с карты на карту"
    assert next(name_transactions) == "Перевод организации"


def test_transaction_descriptions_empty_list():
    name_transactions = transaction_descriptions([])
    assert next(name_transactions) == "Транзакций не обнаружено"

    name_transactions = transaction_descriptions([{}])
    assert next(name_transactions) == "Транзакций не обнаружено"


def test_card_number_generator():
    card_number = card_number_generator(1, 3)
    assert next(card_number) == '0000 0000 0000 0001'
    assert next(card_number) == '0000 0000 0000 0002'
    assert next(card_number) == '0000 0000 0000 0003'


def test_card_number_generator_formatting_correctness():
    card_number = card_number_generator(0, 0)
    assert next(card_number) == '0000 0000 0000 0000'


def test_card_number_generator_extreme_values():
    card_number = card_number_generator(9999999999999998, 9999999999999999)
    assert next(card_number) == '9999 9999 9999 9998'
    assert next(card_number) == '9999 9999 9999 9999'